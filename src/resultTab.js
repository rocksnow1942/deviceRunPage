...
export class ResultTab {
        ...
        switch (action) {
          ...
          case "dataStore.getDataFromIndexes":
            ...
            if (
              _.intersection(
                ids,
                currentids.map((id) => this.data[id]._id)
              ).length
            ) {
  ...
  getCurrentSelection() {
    return this.resultSelect
      .val()
      .map((e) => this.page * this.perPage + parseInt(e));
  }
  ...
  fetchNew() {
    ...
    if (this.data.length > 0) {
      ...
      this.send({
        action: "dataStore.getDataAfterIndex",
        index: this.data[0]._id,
        pwd: "",
        returnRaw: true,
      });
    } else {
  ...
  fetchDataFromIndexes(indexes) {
    this.showRefreshBtn("on");
    this.send({
      action: "dataStore.getDataFromIndexes",
      indexes,
      pwd: "",
      returnRaw: true,
    });
  ...
  addEventListener() {
    this.nextBtn.on("click", (e) => {
      ...
      if (
        !this.allFetched &&
        (this.page + 2) * this.perPage > this.data.length
      ) {
    ...
    $("#result-delete").on("click", (e) => {
      ...
      if (index.length > 0) {
        this.app.confirmDialog.confirm(
          `<h4>Are you sure to delete <strong>${index.length}</strong> data?</h4>`,
          () => {
            index.forEach((idx) => {
              this.send({
                action: "dataStore.deleteDataByIndex",
                index: this.data[idx]._id,
              });
            });
    ...
    $("#result-repredict").on("click", (e) => {
      ...
      if (index.length > 0) {
        index.forEach((idx) => {
          this.send({
            action: "dataProcess.rePredictResult",
            index: this.data[idx]._id,
          });
        });
      }
    ...
    this.resultSelect.on("change", (e) => {
      ...
      if (idx.length) {
        ...
        if (missingDataIdx.length) {
          this.fetchDataFromIndexes(
            missingDataIdx.map((id) => this.data[id]._id)
          );
        }
        // update plot
    ...
    $("#result-save-btn").on("click", (e) => {
      ...
      idx.forEach((e, i, arr) => {
        ...
        this.send({
          action: "dataStore.editMeta",
          index: this.data[e]._id,
          meta: this.data[e].meta,
        });
      });
      if (idx.length) {
        this.updatePage();
    ...
    $("#result-upload-btn").on("click", (e) => {
      ...
      idx.forEach((e, i, arr) => {
        ...
        setTimeout(() => {
          this.send({
            action: "dataStore.getDataFromIndex",
            index: index,
            returnRaw: true,
          });
        }, 5000);
  ...
  updatePeak(idx, channel, j) {
    ...
    let voltage = Array.from(
      { length: rawdata[0][2] },
      (_, k) => rawdata[0][0] + k * step
    );
    let fit = this.data[idx].data.scan[channel].fit[j];
    let time = this.data[idx].data.scan[channel].time[j];
    if (rawdata && fit) {
      this.peak.options.title.text = [
        `${name} - ${channel}`,
        `${time.toFixed(2)} min - ${fit.pc.toFixed(2)}uA`,
      ];
      this.peak.data.datasets[0].data = _.zip(
        voltage,
        rawdata[1]
      ).map(([x, y]) => ({ x, y }));
      let pv = fit.pv;
      ...
      let bp = k * pv + b;
      this.peak.data.datasets[1].data = [
        { x: pv, y: bp },
        { x: pv, y: bp + pc },
      ];
      this.peak.data.datasets[2].data = _.zip(fit.fx, fit.fy).map(([x, y]) => ({
        x,
        y,
      })); //[{x:fit.fx[0],y:fit.fy[0]},{x:fit.fx[0],y:fit.fy[0]}]
      this.peak.update();
    }
  }
    ...
    return indexes.flatMap((dataIndex, i) => {
      ...
      let currentData = d.data
        ? _.map(d.data.scan, (data, channel) => {
            //DEMO: scale data
            let xyData = _.zip(data.time, data.fit).map(([t, fit]) => ({
              x: t ? t.toFixed(2) / scaleRatio : 0,
              y: fit ? fit.pc.toFixed(2) : 0,
            }));
      ...
      let tempData = {
        ...
        data: d.data
          ? _.zip(
              d.data.temperature.time,
              d.data.temperature.data
            ).map(([t, temp]) => ({
              x: t ? t.toFixed(2) / scaleRatio : 0,
              y: temp ? temp.toFixed(2) : 0,
            }))
          : [],
      };
      ...
      let fluidFillData =
        d.data && indexes.length == 1
          ? _.map(d.data.fluidFill, (data, channel) => {
              //DEMO: scale data
              let xyData = _.zip(data.time, data.data).map(([t, d]) => ({
                x: t ? t.toFixed(2) / scaleRatio : 0,
                y: d ? d.toFixed(2) : 0,
              }));
      ...
      return [...currentData, ...fluidFillData, tempData];
    });
  }
      ...
      options: {
        ...
        tooltips: {
          ...
          callbacks: {
            ...
            afterLabel: (tooltipItem, data) => {
              return ` ${data.datasets[tooltipItem.datasetIndex].dataType} : ${
                data.datasets[tooltipItem.datasetIndex].data[tooltipItem.index]
                  .y
              }`;
            },
        ...
        onHover: (e, ae) => {
          if (ae.length) {
            ...
            if (
              this.trace.data.datasets[ae[0]._datasetIndex].yAxisID == "current"
            ) {
              // console.log(ae[0]._datasetIndex,ae[0]._index);
              let dataIndex = this.trace.data.datasets[ae[0]._datasetIndex]
                .dataIndex;
              let channel = this.trace.data.datasets[ae[0]._datasetIndex]
                .channel;
